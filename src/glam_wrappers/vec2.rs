...
#[cfg_attr(
    ...
    rustpython_vm::pyclass(module = "_geomanpy", name = "Vec2")
)]
...
mod pyo3_impl {
    ...
    impl PyDVec2 {
        ...
        #[staticmethod]
        #[inline]
        fn splat(v: f64) -> Self { Self(DVec2::splat(v)) }

        #[staticmethod]
        #[inline]
        fn from_array(a: [f64; 2]) -> Self { Self(DVec2::from_array(a)) }

        #[staticmethod]
        #[inline]
        fn from_numpy(array: PyArrayLike1<'_, f64, AllowTypeChange>) -> PyResult<Self> {
            Ok(Self(DVec2::from_array(extract_numpy_vector::<2>(array, "Vec2")?)))
        }

        #[inline]
        fn to_array(&self) -> [f64; 2] { self.0.to_array() }
        #[inline]
        ...
        #[inline]
        fn extend(&self, z: f64) -> PyDVec3 { PyDVec3(self.0.extend(z)) }
    }
    ...
    impl PyDVec2 {
        ...
        #[inline]
        fn perp(&self) -> Self { Self(self.0.perp()) }
        #[inline]
        fn perp_dot(&self, rhs: Self) -> f64 { self.0.perp_dot(rhs.0) }
        #[inline]
        ...
        #[inline]
        fn rotate(&self, rhs: Self) -> Self { Self(self.0.rotate(rhs.0)) }
    }
    ...
    impl PyDVec2 {
        ...
        #[inline]
        fn move_towards(&self, rhs: Self, d: f64) -> Self { Self(self.0.move_towards(rhs.0, d)) }
        #[inline]
    ...
    impl PyDVec2 {
        ...
        #[inline]
        fn clamp_length(&self, min: f64, max: f64) -> Self { Self(self.0.clamp_length(min, max)) }
        #[inline]
    ...
    impl PyDVec2 {
        ...
        #[inline]
        fn angle_to(&self, rhs: Self) -> f64 { self.0.angle_to(rhs.0) }
        #[inline]
    ...
    #[pymethods]
    impl PyDVec2 {
        fn __repr__(&self) -> String { format!("Vec2({}, {})", self.0.x, self.0.y) }
        ...
        fn __add__(&self, other: &Bound<'_, PyAny>) -> PyResult<Self> {
            if let Ok(v) = other.extract::<Self>() { Ok(Self(self.0 + v.0)) }
            else if let Ok(s) = other.extract::<f64>() { Ok(Self(self.0 + s)) }
        ...
        fn __rsub__(&self, other: &Bound<'_, PyAny>) -> PyResult<Self> {
            if let Ok(v) = other.extract::<Self>() { Ok(Self(v.0 - self.0)) }
            else if let Ok(s) = other.extract::<f64>() { Ok(Self(DVec2::splat(s) - self.0)) }
            else { Err(pyo3::exceptions::PyTypeError::new_err("unsupported operand type for -")) }
        }
...
mod rustpython_impl {
    ...
    impl Constructor for PyDVec2 {
        ...
        fn py_new(_cls: &Py<PyType>, args: FuncArgs, vm: &VirtualMachine) -> PyResult<Self> {
            let (x, y) = match args.args.len() {
                ...
                n => return Err(vm.new_type_error(format!("Vec2() takes 0 or 2 positional arguments, got {n}"))),
            };
    ...
    impl PyDVec2 {
        ...
        #[pymethod]
        fn dot(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<f64> { Ok(self.0.dot(extract(&rhs, vm)?)) }
        #[pymethod]
        fn perp(&self) -> Self { Self(self.0.perp()) }
        #[pymethod]
        fn perp_dot(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<f64> { Ok(self.0.perp_dot(extract(&rhs, vm)?)) }
        #[pymethod]
        fn length(&self) -> f64 { self.0.length() }
        #[pymethod]
        fn length_squared(&self) -> f64 { self.0.length_squared() }
        ...
        fn length_recip(&self) -> f64 { self.0.length_recip() }
        #[pymethod]
        fn distance(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<f64> { Ok(self.0.distance(extract(&rhs, vm)?)) }
        #[pymethod]
        fn distance_squared(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<f64> { Ok(self.0.distance_squared(extract(&rhs, vm)?)) }
        #[pymethod]
        fn normalize(&self) -> Self { Self(self.0.normalize()) }
        #[pymethod]
        ...
        fn is_normalized(&self) -> bool { self.0.is_normalized() }
        #[pymethod]
        fn rotate(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<Self> { Ok(Self(self.0.rotate(extract(&rhs, vm)?))) }
        #[pymethod]
        fn lerp(&self, rhs: PyObjectRef, s: f64, vm: &VirtualMachine) -> PyResult<Self> { Ok(Self(self.0.lerp(extract(&rhs, vm)?, s))) }
        #[pymethod]
        fn midpoint(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<Self> { Ok(Self(self.0.midpoint(extract(&rhs, vm)?))) }
        #[pymethod]
        fn min(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<Self> { Ok(Self(self.0.min(extract(&rhs, vm)?))) }
        #[pymethod]
        fn max(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<Self> { Ok(Self(self.0.max(extract(&rhs, vm)?))) }
        #[pymethod]
        ...
        fn element_product(&self) -> f64 { self.0.element_product() }
        #[pymethod]
        fn angle_to(&self, rhs: PyObjectRef, vm: &VirtualMachine) -> PyResult<f64> { Ok(self.0.angle_to(extract(&rhs, vm)?)) }
        #[pymethod]
        fn is_finite(&self) -> bool { self.0.is_finite() }
        #[pymethod]
        fn is_nan(&self) -> bool { self.0.is_nan() }
        ...
        #[pyclassmethod]
        fn ZERO(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DVec2::ZERO) }
        #[pyclassmethod]
        fn ONE(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DVec2::ONE) }
        #[pyclassmethod]
        fn X(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DVec2::X) }
        #[pyclassmethod]
        fn Y(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DVec2::Y) }
        #[pyclassmethod]
        fn NEG_X(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DVec2::NEG_X) }
        #[pyclassmethod]
        fn NEG_Y(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DVec2::NEG_Y) }
        #[pyclassmethod]
        fn NEG_ONE(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DVec2::NEG_ONE) }
    }
}
