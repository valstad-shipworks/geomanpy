...
impl From<DQuat> for PyDQuat {
    #[inline]
    fn from(q: DQuat) -> Self { Self(q) }
}

impl From<PyDQuat> for DQuat {
    #[inline]
    fn from(q: PyDQuat) -> Self { q.0 }
}
...
mod pyo3_impl {
    ...
    impl PyDQuat {
        ...
        #[staticmethod]
        #[inline]
        fn from_xyzw(x: f64, y: f64, z: f64, w: f64) -> Self { Self(DQuat::from_xyzw(x, y, z, w)) }
        #[staticmethod]
        #[inline]
        fn from_array(a: [f64; 4]) -> Self { Self(DQuat::from_array(a)) }
        #[staticmethod]
        #[inline]
        fn from_numpy(array: PyArrayLike1<'_, f64, AllowTypeChange>) -> PyResult<Self> {
            Ok(Self(DQuat::from_array(extract_numpy_vector::<4>(array, "Quat")?)))
        }
        #[staticmethod]
        #[inline]
        fn from_axis_angle(axis: PyDVec3, angle: f64) -> Self { Self(DQuat::from_axis_angle(axis.0, angle)) }
        #[staticmethod]
        #[inline]
        fn from_scaled_axis(v: PyDVec3) -> Self { Self(DQuat::from_scaled_axis(v.0)) }
        #[staticmethod]
        #[inline]
        fn from_rotation_x(angle: f64) -> Self { Self(DQuat::from_rotation_x(angle)) }
        #[staticmethod]
        #[inline]
        fn from_rotation_y(angle: f64) -> Self { Self(DQuat::from_rotation_y(angle)) }
        #[staticmethod]
        #[inline]
        fn from_rotation_z(angle: f64) -> Self { Self(DQuat::from_rotation_z(angle)) }
        #[staticmethod]
        #[inline]
        ...
        #[staticmethod]
        #[inline]
        fn from_mat3(mat: PyDMat3) -> Self { Self(DQuat::from_mat3(&mat.0)) }
        #[staticmethod]
        #[inline]
        fn from_mat4(mat: PyDMat4) -> Self { Self(DQuat::from_mat4(&mat.0)) }
        #[staticmethod]
        #[inline]
        fn from_affine3(a: PyDAffine3) -> Self { Self(DQuat::from_affine3(&a.0)) }
        #[staticmethod]
        #[inline]
        fn from_rotation_arc(from: PyDVec3, to: PyDVec3) -> Self { Self(DQuat::from_rotation_arc(from.0, to.0)) }
        #[staticmethod]
        #[inline]
        fn from_rotation_arc_colinear(from: PyDVec3, to: PyDVec3) -> Self { Self(DQuat::from_rotation_arc_colinear(from.0, to.0)) }
        #[staticmethod]
        #[inline]
        fn from_rotation_arc_2d(from: PyDVec2, to: PyDVec2) -> Self { Self(DQuat::from_rotation_arc_2d(from.0, to.0)) }
        #[staticmethod]
        #[inline]
        fn look_to_lh(dir: PyDVec3, up: PyDVec3) -> Self { Self(DQuat::look_to_lh(dir.0, up.0)) }
        #[staticmethod]
        #[inline]
        fn look_to_rh(dir: PyDVec3, up: PyDVec3) -> Self { Self(DQuat::look_to_rh(dir.0, up.0)) }
        #[staticmethod]
        #[inline]
        fn look_at_lh(eye: PyDVec3, center: PyDVec3, up: PyDVec3) -> Self { Self(DQuat::look_at_lh(eye.0, center.0, up.0)) }
        #[staticmethod]
        #[inline]
        fn look_at_rh(eye: PyDVec3, center: PyDVec3, up: PyDVec3) -> Self { Self(DQuat::look_at_rh(eye.0, center.0, up.0)) }
    }
    ...
    impl PyDQuat {
        #[inline]
        fn conjugate(&self) -> Self { Self(self.0.conjugate()) }
        #[inline]
        ...
        #[inline]
        fn mul_quat(&self, rhs: Self) -> Self { Self(self.0.mul_quat(rhs.0)) }
    }

    #[pymethods]
    impl PyDQuat {
        #[inline]
        fn lerp(&self, end: Self, s: f64) -> Self { Self(self.0.lerp(end.0, s)) }
        #[inline]
        fn slerp(&self, end: Self, s: f64) -> Self { Self(self.0.slerp(end.0, s)) }
        #[inline]
        fn angle_between(&self, rhs: Self) -> f64 { self.0.angle_between(rhs.0) }
        #[inline]
        fn rotate_towards(&self, rhs: Self, max_angle: f64) -> Self { Self(self.0.rotate_towards(rhs.0, max_angle)) }
    }
    ...
    impl PyDQuat {
        ...
        #[inline]
        fn to_scaled_axis(&self) -> PyDVec3 { PyDVec3(self.0.to_scaled_axis()) }
        #[inline]
        ...
        #[inline]
        fn xyz(&self) -> PyDVec3 { PyDVec3(self.0.xyz()) }
    }
    ...
    impl PyDQuat {
        ...
        #[inline]
        fn is_near_identity(&self) -> bool { self.0.is_near_identity() }
        #[inline]
    ...
    impl PyDQuat {
        #[classattr]
        #[pyo3(name = "IDENTITY")]
        fn identity_const() -> Self { Self(DQuat::IDENTITY) }
        #[classattr]
        #[pyo3(name = "NAN")]
        fn nan_const() -> Self { Self(DQuat::NAN) }
    }

    #[pymethods]
    impl PyDQuat {
        fn __repr__(&self) -> String { format!("Quat({}, {}, {}, {})", self.0.x, self.0.y, self.0.z, self.0.w) }
        ...
        fn __mul__<'py>(&self, other: &Bound<'py, PyAny>) -> PyResult<Py<PyAny>> {
            ...
            if let Ok(q) = other.extract::<Self>() {
                Ok(Self(self.0.mul_quat(q.0)).into_pyobject(py)?.into_any().unbind())
            } else if let Ok(v) = other.extract::<PyDVec3>() {
                Ok(PyDVec3(self.0.mul_vec3(v.0)).into_pyobject(py)?.into_any().unbind())
        ...
        fn __rmul__(&self, other: &Bound<'_, PyAny>) -> PyResult<Self> {
            if let Ok(s) = other.extract::<f64>() { Ok(Self(self.0 * s)) }
...
mod rustpython_impl {
    ...
    impl Constructor for PyDQuat {
        ...
        fn py_new(_cls: &Py<PyType>, args: FuncArgs, vm: &VirtualMachine) -> PyResult<Self> {
            let (x, y, z, w) = match args.args.len() {
                ...
                n => return Err(vm.new_type_error(format!("Quat() takes 0 or 4 positional arguments, got {n}"))),
            };
    ...
    impl PyDQuat {
        ...
        fn from_axis_angle(axis: PyObjectRef, angle: f64, vm: &VirtualMachine) -> PyResult<Self> {
            Ok(Self(DQuat::from_axis_angle(extract_vec3(&axis, vm)?, angle)))
        }
        #[pystaticmethod]
        ...
        #[pystaticmethod]
        fn from_rotation_arc(from: PyObjectRef, to: PyObjectRef, vm: &VirtualMachine) -> PyResult<Self> {
            Ok(Self(DQuat::from_rotation_arc(extract_vec3(&from, vm)?, extract_vec3(&to, vm)?)))
        }
        ...
        #[pyclassmethod]
        fn IDENTITY(_cls: rustpython_vm::builtins::PyTypeRef) -> Self { Self(DQuat::IDENTITY) }
    }
}
